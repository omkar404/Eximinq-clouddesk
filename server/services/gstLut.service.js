...
const SERVICE_SLUG = "gst-lut-filing";
...
async function getDefinition() {
  ...
  if (!definition) throw httpError(404, "GST LUT Undertaking service is unavailable");
  return definition;
}
function calculatePricing(config, requestType) {
  const selected = config?.pricing?.[requestType];
  if (!selected) throw httpError(400, "Pricing is unavailable for the selected LUT filing type");
  const officialFee = Number(selected.officialFee || 0);
  const draftingFee = Number(selected.draftingFee || 0);
  const transmissionShield = Number(selected.transmissionShield || 0);
  const serviceCharge = draftingFee + transmissionShield;
  const gstRate = Number(selected.gstRate || 0);
  const gst = Number(((serviceCharge * gstRate) / 100).toFixed(2));
  return { currency: config.currency || "INR", officialFee, draftingFee, transmissionShield,
    serviceCharge, gstRate, gst, total: Number((officialFee + serviceCharge + gst).toFixed(2)) };
}
export async function getGstLutConfiguration(userId) {
...
export async function getGstLutQuote(userId, requestType) {
  const { service, financialContext } = await getGstLutConfiguration(userId);
...
export async function saveGstLutDraft(userId, payload) {
  const pricingSnapshot = await getGstLutQuote(userId, payload.requestType);
  ...
  if (!current) throw httpError(404, "GST LUT draft not found");
...
export async function submitGstLut(userId, payload) {
  const [definition, pricingSnapshot] = await Promise.all([getDefinition(), getGstLutQuote(userId, payload.requestType)]);
  if (!payload.requestId) throw httpError(422, "Save the LUT request before submitting it");
  ...
  if (missing.length) throw httpError(422, "Upload all required LUT documents", { documents: missing.map((item) => item.id) });
...
export const getGstLutRequests = (userId) => listServiceRequests(userId, SERVICE_SLUG);
export async function getGstLutLedger(userId) {
...
export async function storeGstLutDocument({ userId, requestId, documentKey, file }) {
  const [request, definition] = await Promise.all([findServiceRequest(requestId, userId, SERVICE_SLUG), getDefinition()]);
  if (!request) throw httpError(404, "GST LUT draft not found");
  if (request.status !== "DRAFT") throw httpError(409, "Documents cannot be changed after submission");
  if (!definition.config?.documents?.some((item) => item.id === documentKey)) throw httpError(400, "Unknown LUT document type");
...
export const getGstLutDocument = (userId, requestId, documentKey) => findServiceDocument(requestId, documentKey, userId, SERVICE_SLUG);
export const removeGstLutDocument = (userId, requestId, documentKey) => deleteServiceDocument(requestId, documentKey, userId, SERVICE_SLUG);
