...
async function getDefinition() {
  ...
  if (!definition) throw httpError(404, "GST Returns & ITC Audit service is unavailable");
  return definition;
}
function calculatePricing(config, returnType) {
  const selected = config?.pricing?.[returnType];
  if (!selected) throw httpError(400, "Pricing is unavailable for the selected GST filing type");
  ...
  return { currency: config.currency || "INR", officialFee, draftingFee, shieldPremium, serviceCharge,
    gstRate, gst, total: Number((officialFee + serviceCharge + gst).toFixed(2)) };
}
export async function getGstReturnsConfiguration(userId) {
...
export async function getGstReturnsQuote(userId, returnType) {
  const { service, financialContext } = await getGstReturnsConfiguration(userId);
  const quote = calculatePricing(service, returnType);
...
export async function saveGstReturnsDraft(userId, payload) {
  const pricingSnapshot = await getGstReturnsQuote(userId, payload.returnType);
  ...
  if (!current) throw httpError(404, "GST return draft not found");
...
export async function submitGstReturns(userId, payload) {
  const [definition, pricingSnapshot] = await Promise.all([getDefinition(), getGstReturnsQuote(userId, payload.returnType)]);
  if (!payload.requestId) throw httpError(422, "Save the GST request before submitting it");
  ...
  const missing = (definition.config?.documents || []).filter((item) => item.required &&
    (!item.returnTypes || item.returnTypes.includes(payload.returnType)) && !uploadedKeys.has(item.id));
  if (missing.length) throw httpError(422, "Upload all required GST records", { documents: missing.map((item) => item.id) });
...
export const getGstReturnsRequests = (userId) => listServiceRequests(userId, SERVICE_SLUG);
export async function getGstReturnsLedger(userId) {
...
export async function storeGstReturnsDocument({ userId, requestId, documentKey, file }) {
  const [request, definition] = await Promise.all([findServiceRequest(requestId, userId, SERVICE_SLUG), getDefinition()]);
  if (!request) throw httpError(404, "GST return draft not found");
  if (request.status !== "DRAFT") throw httpError(409, "Documents cannot be changed after submission");
  if (!definition.config?.documents?.some((item) => item.id === documentKey)) throw httpError(400, "Unknown GST document type");
...
export const getGstReturnsDocument = (userId, requestId, documentKey) => findServiceDocument(requestId, documentKey, userId, SERVICE_SLUG);
export const removeGstReturnsDocument = (userId, requestId, documentKey) => deleteServiceDocument(requestId, documentKey, userId, SERVICE_SLUG);
