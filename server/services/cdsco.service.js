...
const SERVICE_SLUG = "cdsco-drug-control";
...
async function getDefinition() {
  ...
  if (!definition) throw httpError(404, "CDSCO Import Authorization service is unavailable");
  return definition;
}
function calculatePricing(config, requestMode, deviceClass) {
  const selected = config?.pricing?.[requestMode];
  if (!selected) throw httpError(400, "Pricing is unavailable for the selected CDSCO authorization type");
  const officialFee = Number(selected.officialFee || 0);
  const draftingFee = requestMode === "medical-device" && ["C", "D"].includes(deviceClass)
    ? Number(selected.highRiskDraftingFee || selected.draftingFee || 0)
    : Number(selected.draftingFee || 0);
  const shieldPremium = Number(selected.shieldPremium || 0);
  const serviceCharge = draftingFee + shieldPremium;
  const gstRate = Number(selected.gstRate || 0);
  const gst = Number(((serviceCharge * gstRate) / 100).toFixed(2));
  return { currency: config.currency || "INR", officialFee, draftingFee, shieldPremium,
...
export async function getCdscoQuote(userId, requestMode, deviceClass) {
  const { service, financialContext } = await getCdscoConfiguration(userId);
  const quote = calculatePricing(service, requestMode, deviceClass);
...
export async function saveCdscoDraft(userId, payload) {
  const pricingSnapshot = await getCdscoQuote(userId, payload.requestMode, payload.deviceClass);
  ...
  if (!current) throw httpError(404, "CDSCO authorization draft not found");
...
export async function submitCdsco(userId, payload) {
  const [definition, pricingSnapshot] = await Promise.all([getDefinition(), getCdscoQuote(userId, payload.requestMode, payload.deviceClass)]);
  if (!payload.requestId) throw httpError(422, "Save the CDSCO request before submitting it");
  ...
  const missing = (definition.config?.documents || []).filter((item) => item.required &&
    (!item.requestModes || item.requestModes.includes(payload.requestMode)) && !uploadedKeys.has(item.id));
  if (missing.length) throw httpError(422, "Upload all required CDSCO documents", { documents: missing.map((item) => item.id) });
  ...
  try {
    ...
    if (error.code?.startsWith("INSUFFICIENT_")) throw httpError(402, "Available balance is insufficient", { code: error.code, ...error.balanceDetails });
    if (error.code === "REQUEST_NOT_EDITABLE") throw httpError(409, error.message);
    throw error;
  }
}
export const getCdscoRequests = (userId) => listServiceRequests(userId, SERVICE_SLUG);
export async function getCdscoLedger(userId) {
  const [finances, transactions] = await Promise.all([findFinancialContext(userId), listFinancialTransactions(userId)]);
  return { balances: { walletBalance: Number(finances.wallet_balance || 0), creditLineBalance: Number(finances.credit_line_balance || 0), creditLimit: Number(finances.credit_limit || 0) },
    transactions: transactions.map((item) => ({ id: item.id, accountType: item.account_type, transactionType: item.transaction_type,
      amount: Number(item.amount), balanceAfter: Number(item.balance_after), description: item.description, status: item.status,
      transactionDate: item.created_at, requestCode: item.request_code, serviceName: item.service_name })) };
}
export async function storeCdscoDocument({ userId, requestId, documentKey, file }) {
  const [request, definition] = await Promise.all([findServiceRequest(requestId, userId, SERVICE_SLUG), getDefinition()]);
  if (!request) throw httpError(404, "CDSCO authorization draft not found");
  if (request.status !== "DRAFT") throw httpError(409, "Documents cannot be changed after submission");
  if (!definition.config?.documents?.some((item) => item.id === documentKey)) throw httpError(400, "Unknown CDSCO document type");
...
export const getCdscoDocument = (userId, requestId, documentKey) => findServiceDocument(requestId, documentKey, userId, SERVICE_SLUG);
export const removeCdscoDocument = (userId, requestId, documentKey) => deleteServiceDocument(requestId, documentKey, userId, SERVICE_SLUG);
