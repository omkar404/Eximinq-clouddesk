...
const SERVICE_SLUG = "certificate-of-origin";

export async function findCertificateOfOriginDefinition() {
...
export async function findClientFinancialContext(userId) {
  const result = await pool.query(
    `SELECT COALESCE(w.balance, 0)::numeric AS wallet_balance,
            COALESCE(c.available_balance, 0)::numeric AS credit_line_balance,
...
export async function createCertificateOfOriginRequest({
  userId,
  status,
  payload,
  pricingSnapshot
  ...
  const result = await pool.query(
    ...
    [userId, SERVICE_SLUG, status, payload, pricingSnapshot]
  );

  return result.rows[0];
}

export async function updateCertificateOfOriginRequest({
  requestId,
  userId,
  status,
  payload,
  ...
  const result = await pool.query(
    `UPDATE service_requests
        SET status = $3,
            ...
            updated_at = NOW()
      WHERE id = $1 AND user_id = $2 AND service_slug = $6
      RETURNING id, request_code, service_slug, status, payload, pricing_snapshot,
                submitted_at, created_at, updated_at`,
    [requestId, userId, status, payload, pricingSnapshot, SERVICE_SLUG]
  );

  return result.rows[0] || null;
}

export async function findCertificateOfOriginRequest(requestId, userId) {
  const result = await pool.query(
    `SELECT id, request_code, service_slug, status, payload, pricing_snapshot,
            submitted_at, created_at, updated_at
       FROM service_requests
      WHERE id = $1 AND user_id = $2 AND service_slug = $3`,
    [requestId, userId, SERVICE_SLUG]
  );

  return result.rows[0] || null;
}

export async function listCertificateOfOriginRequests(userId) {
  const result = await pool.query(
...
export async function upsertRequestDocument({
  requestId,
  documentKey,
  originalName,
  storedName,
  ...
  const result = await pool.query(
    `INSERT INTO service_request_documents(
     ...
     ON CONFLICT(request_id, document_key) DO UPDATE SET
       ...
       uploaded_at = NOW()
     RETURNING id, request_id, document_key, original_name, stored_name,
...
export async function findRequestDocument(requestId, documentKey, userId) {
  const result = await pool.query(
    `SELECT d.*
       FROM service_request_documents d
       JOIN service_requests sr ON sr.id = d.request_id
      WHERE d.request_id = $1 AND d.document_key = $2
        AND sr.user_id = $3 AND sr.service_slug = $4`,
    [requestId, documentKey, userId, SERVICE_SLUG]
  );

  return result.rows[0] || null;
}

export async function deleteRequestDocument(requestId, documentKey, userId) {
  const result = await pool.query(
    `DELETE FROM service_request_documents d
      USING service_requests sr
      WHERE d.request_id = $1 AND d.document_key = $2
        AND sr.id = d.request_id AND sr.user_id = $3 AND sr.service_slug = $4
        AND sr.status = 'DRAFT'
      RETURNING d.*`,
    [requestId, documentKey, userId, SERVICE_SLUG]
  );
...
export async function listFinancialTransactions(userId, limit = 50) {
  const result = await pool.query(
...
export async function submitRequestAndDeductBalances({
  userId,
  requestId,
  payload,
  pricingSnapshot,
  ...
  try {
    ...
    const balancesResult = await client.query(
      `SELECT w.balance, c.credit_limit, c.available_balance
         FROM wallets w
    ...
    const creditDeduction = Number(
      (Number(pricingSnapshot.serviceCharge) + Number(pricingSnapshot.gst)).toFixed(2)
    );
    ...
    if (requestId) {
      const requestResult = await client.query(
        `UPDATE service_requests
            SET status = 'SUBMITTED', payload = $3, pricing_snapshot = $4,
                submitted_at = NOW(), updated_at = NOW()
          WHERE id = $1 AND user_id = $2 AND service_slug = $5 AND status = 'DRAFT'
          RETURNING id, request_code, service_slug, status, payload, pricing_snapshot,
                    submitted_at, created_at, updated_at`,
        [requestId, userId, payload, pricingSnapshot, SERVICE_SLUG]
      );
      request = requestResult.rows[0];
    } else {
      const requestResult = await client.query(
        `INSERT INTO service_requests(
           user_id, service_slug, status, payload, pricing_snapshot, submitted_at
         )
         VALUES($1, $2, 'SUBMITTED', $3, $4, NOW())
         RETURNING id, request_code, service_slug, status, payload, pricing_snapshot,
                   submitted_at, created_at, updated_at`,
        [userId, SERVICE_SLUG, payload, pricingSnapshot]
      );
      request = requestResult.rows[0];
    }
    if (!request) {
    ...
    await client.query(
      `INSERT INTO financial_transactions(
         user_id, service_request_id, service_slug, account_type,
    ...
    return {
      ...
      balances: {
        ...
        creditLineBalance: creditAfter,
        creditLimit: Number(balances.credit_limit)
      }
    };
  } catch (error) {
