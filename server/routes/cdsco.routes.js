...
import { deleteDocument, downloadDocument, getConfiguration, getLedger, getQuote, getRequests,
  saveDraft, submitRequest, uploadDocument } from "../controllers/cdsco.controller.js";
...
const upload = multer({ storage: multer.diskStorage({ destination: uploadDirectory,
  filename(_req, file, callback) { callback(null, `${crypto.randomUUID()}${extname(file.originalname).toLowerCase()}`); } }),
  limits: { fileSize: 20 * 1024 * 1024 }, fileFilter(_req, file, callback) { const valid = allowed.has(file.mimetype); callback(valid ? null : new Error("Unsupported document type"), valid); } });
export function createCdscoRouter({ requireAuth }) {
