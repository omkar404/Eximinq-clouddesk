...
import { deleteDocument, downloadDocument, getConfiguration, getLedger, getQuote, getRequests,
  saveDraft, submitRequest, uploadDocument } from "../controllers/gstReturnsAudit.controller.js";
const uploadDirectory = resolve("server/uploads/gst-returns-audit");
mkdirSync(uploadDirectory, { recursive: true });
const allowed = new Set(["application/pdf", "text/csv", "application/json", "application/vnd.ms-excel",
  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "image/jpeg", "image/png"]);
const upload = multer({ storage: multer.diskStorage({ destination: uploadDirectory, filename(_req, file, callback) {
  callback(null, `${crypto.randomUUID()}${extname(file.originalname).toLowerCase()}`);
} }), limits: { fileSize: 15 * 1024 * 1024 }, fileFilter(_req, file, callback) {
  const valid = allowed.has(file.mimetype); callback(valid ? null : new Error("Unsupported GST document type"), valid);
} });
export function createGstReturnsAuditRouter({ requireAuth }) {
