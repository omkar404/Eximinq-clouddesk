...
import {
  ...
  submitBis,
  uploadBisDocument,
} from "../../services/bisRegistrationService";
...
export default function BisRegistrationWorkflow({ service, onBack }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const config = await getBisConfiguration();
        ...
        let pricing = {};
        try {
          pricing = await getBisQuote(mode);
        } catch {
          pricing = {};
        }
        if (!active) return;
        ...
        setRequestMode(mode);
        setIsStandard(saved.isStandard || "");
        setModelName(saved.modelName || "");
        setVariantCount(saved.variantCount || 1);
        setFiles(restored);
        setError("");
      } catch {
        if (active) setError("Unable to load the BIS Registration workflow.");
      } finally {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      () =>
        getBisQuote(requestMode)
          .then((data) => {
            setQuote(data);
            setError("");
          })
          .catch(() => setError("Unable to calculate BIS pricing.")),
      180,
    );
  ...
  const documents = useMemo(
    () =>
      (config?.documents || []).filter(
        (item) => !item.showFor || item.showFor.includes(requestMode),
      ),
    [config, requestMode],
  );
  const isRequired = (document) =>
    document.required || document.requiredFor?.includes(requestMode);
  const payload = () => ({
    requestId,
    requestMode,
    isStandard,
  ...
  const valid =
    Boolean(isStandard.trim() && modelName.trim()) &&
  ...
  const site =
    requestMode === "FMCS"
      ...
      : "Domestic / SEZ Manufacturing Site";

  async function ensureDraft() {
    if (requestId) return requestId;
    const data = await saveBisDraft(payload());
  ...
  async function selectFile(key, event) {
    ...
    try {
      ...
      const data = await uploadBisDocument(id, key, file);
      setFiles((current) => ({
        ...current,
        [key]: {
          status: "Uploaded",
  ...
  async function removeFile(key) {
    try {
      setUploading(key);
      await removeBisDocument(requestId, key);
  ...
  async function submit() {
    ...
    try {
      ...
      const data = await submitBis({ ...payload(), requestId: id });
          ...
          <div className="mb-3 flex gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-3 text-blue-900">
            ...
            <div>
              ...
              <p className="text-[11px] text-blue-600">
                Continue your BIS certification request.
              </p>
            </div>
          </div>
        )}
        <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_60px_rgba(15,23,42,.08)]">
            ...
            <div className="space-y-5">
              ...
              <div>
                <h2 className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em]">
                  <Upload size={14} /> Required conformity evidence
                </h2>
              ...
              <div className="flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
                ...
                <p className="text-[11px] font-semibold">
                  BIS certification evidence must match the selected
                  certification route and declared product variants.
                  Inconsistent declarations can delay laboratory and factory
                  review.
                </p>
              </div>
            </div>
            <aside className="overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl xl:sticky xl:top-24 xl:self-start">
              ...
              <div className="space-y-5 p-5">
                <Ledger
                  ...
                  lines={[["BIS Official Fee", quote.officialFee]]}
                ...
                <Ledger
                  ...
                  lines={[
                    ...
                    ["Compliance Shield", quote.shieldPremium],
                    ["GST", quote.gst],
                  ]}
          ...
          <footer className="flex flex-col justify-between gap-3 border-t bg-slate-50 p-5 sm:flex-row">
            ...
            <div className="flex gap-3">
              ...
              >
                <FileCheck2 size={16} />
                {valid ? "Submit BIS Request" : "Incomplete Application"}
                <ArrowRight size={16} />
              </button>
            </div>
          </footer>
