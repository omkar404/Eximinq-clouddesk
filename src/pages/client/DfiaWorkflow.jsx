...
export default function DfiaWorkflow({ onBack }) {
  ...
  const activeDocuments = useMemo(
    () =>
      (configuration?.documents || []).filter(
        (item) =>
          !item.visibleWhen ||
          String(form[item.visibleWhen.field]) ===
            String(item.visibleWhen.equals),
      ),
  ...
  async function updateField(field, value) {
    ...
    if (field === "dfiaType" && value === "transferability")
      next.dutyValue = "";
    setForm(next);
    try {
  ...
  function validationMessage() {
    if (!form.raOffice?.trim())
      return "Select the DGFT Regional Authority office.";
    if (!form.sionNo?.trim())
      return "Enter the SION serial number or norms reference.";
    if (form.dfiaType === "issuance" && Number(form.dutyValue) <= 0)
      return "Enter a valid estimated entitlement value.";
    const uploaded = new Set(documents.map(documentKey));
    const missing = activeDocuments.find(
  ...
  async function uploadDocument(key, event) {
    ...
    try {
      ...
      setDocuments((current) => [
        ...current.filter((item) => documentKey(item) !== key),
        response.document,
      ]);
    } catch (error) {
      Swal.fire(
  ...
  async function submit() {
    ...
    if (missing)
      return Swal.fire({
        icon: "error",
        title: "DFIA application is incomplete",
        text: missing,
        confirmButtonColor: "#2952ff",
      });
    try {
      ...
      await Swal.fire({
        icon: "success",
        title: "DFIA application submitted",
        text: `${response.request.request_code} has been sent to the admin team.`,
        confirmButtonColor: "#2952ff",
      });
      onBack();
    } catch (error) {
      if (error.response?.status === 402) {
        const result = await Swal.fire({
          ...
          text: "Your Wallet or Credit Line balance is insufficient for this DFIA request.",
        ...
        Swal.fire(
          "Unable to submit DFIA",
          errorText(error, "Please verify the application and try again."),
          "error",
        );
    } finally {
      setBusy(false);
  ...
  if (loading || !configuration)
    return (
      <div className="flex min-h-[520px] items-center justify-center text-slate-500">
        <LoaderCircle className="mr-2 animate-spin" /> Loading DFIA workflow…
      </div>
    );
      ...
      <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_60px_rgba(15,23,42,.08)]">
        <header className="flex items-start justify-between border-b px-5 py-5 md:px-7">
          <div className="flex gap-4">
            ...
            <div>
              ...
              >
                <ArrowLeft size={16} /> Back to Licensing
              </button>
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <h1 className="text-xl font-black">DFIA Authorisation</h1>
                <span className="rounded-full bg-slate-900 px-2.5 py-1 text-[9px] font-black uppercase tracking-widest text-white">
                  Licensing
                </span>
              </div>
              <p className="mt-1 flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-[.16em] text-slate-500">
                <Fingerprint size={11} className="text-[#2952ff]" />
                {draft?.request_code || "Reference generated on save"}
                <Zap size={11} className="text-amber-500" /> Priority SLA
              </p>
            </div>
          </div>
          <button onClick={onBack} aria-label="Close DFIA workflow">
            <X className="text-slate-400" />
          </button>
        </header>
        <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-blue-50/70 px-5 py-3 text-[10px] font-black uppercase tracking-[.14em] text-blue-700">
          <span className="flex items-center gap-2">
            <RefreshCw size={12} /> Exporter profile:{" "}
            {entity.companyName || "Company profile"}
            {form.iecNumber ? ` · IEC ${form.iecNumber}` : ""}
          </span>
          <span>Post-export mode</span>
        </div>
        <div className="grid gap-6 p-5 xl:grid-cols-[minmax(0,1.4fr)_360px] md:p-7">
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-2 rounded-2xl border bg-slate-100 p-1">
              {[
                { id: "issuance", label: "Licence Issuance", Icon: FileCheck },
                { id: "transferability", label: "Transferability", Icon: Link },
              ].map((item) => (
                <button
                  key={item.id}
                  onClick={() => updateField("dfiaType", item.id)}
                  className={`flex items-center justify-center gap-2 rounded-xl px-3 py-3 text-xs font-black uppercase ${form.dfiaType === item.id ? "border border-blue-200 bg-white text-[#2952ff] shadow-sm" : "text-slate-400"}`}
                >
                  <item.Icon size={16} />
                  {item.label}
                </button>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="DGFT RA Office" required>
                <div className="relative">
                  <select
                    value={form.raOffice}
                    onChange={(e) => updateField("raOffice", e.target.value)}
                  >
                    <option value="">Select Regional Authority</option>
                    {RA_OFFICES.map((office) => (
              ...
              <Field label="SION Serial Number" required>
                <input
                  maxLength={100}
                  value={form.sionNo}
                  onChange={(e) =>
                    updateField("sionNo", e.target.value.toUpperCase())
                  }
                  placeholder="E.g. C-123 (Chemicals)"
                />
              </Field>
              {form.dfiaType === "issuance" && (
                <Field
                  label="Estimated Entitlement Value (INR)"
                  required
                ...
                >
                  <input
                    ...
                    onChange={(e) => updateField("dutyValue", e.target.value)}
                    placeholder="Enter total duty value to be issued"
                  />
                  <p className="mt-2 text-[10px] font-medium italic text-blue-600">
                    Success fee: 1% of the transferable duty credit issued
                    post-export.
                  </p>
                </Field>
              )}
              ...
              <div className="space-y-2">
                {activeDocuments.map((item) => {
                  const saved = documents.find(
                    (document) => documentKey(document) === item.id,
                  );
                  return (
                    <article
                      key={item.id}
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <b className="block text-xs">
                          {item.label}
                          <em className="ml-2 not-italic text-[9px] uppercase text-rose-500">
                            Required
                          </em>
                        </b>
                        <span
                      ...
                      {saved && (
                        <>
                          <button
                            onClick={() =>
                              downloadDocument(item.id, documentName(saved))
                            }
                            aria-label={`Download ${item.label}`}
                            className="rounded-lg p-2 text-[#2952ff] hover:bg-blue-50"
                          >
                            <Download size={16} />
                          </button>
                          <button
            ...
            <div className="flex gap-3 rounded-xl border bg-slate-50 p-4 text-[11px] text-slate-600">
              <Info className="shrink-0 text-[#2952ff]" size={16} />
              <p>
                <b>Statutory note:</b> IEC and GST credentials are synchronized
                from the Company Profile. Ensure every Shipping Bill is
                correctly linked to the IEC in DGFT records.
              </p>
            </div>
          </div>
          <aside className="h-fit overflow-hidden rounded-3xl bg-slate-950 text-white shadow-xl xl:sticky xl:top-24">
            ...
            <div className="space-y-4 p-5">
              <Ledger
                label="DGFT official fee — Wallet"
                value={price.officialFee}
              />
              <Ledger
                label="Drafting & audit submission"
                value={price.draftingFee}
              />
              {form.dfiaType === "issuance" && (
                <Ledger label="Success fee (1%)" value={price.successFee} />
              )}
              ...
              <p className="text-[10px] text-slate-500">
                Current balances: Wallet ₹{money(balances.walletBalance)} ·
                Credit ₹{money(balances.creditLineBalance)}
              </p>
            </div>
          </aside>
        </div>
        <footer className="flex flex-col justify-between gap-3 border-t bg-slate-50 p-5 sm:flex-row">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            >
              {valid ? "Process Payment & Submit" : "Review Requirements"}
              <ArrowRight size={16} />
            </button>
          </div>
        </footer>
  ...
  return (
    <label className={`block ${className}`}>
      <span className="mb-2 block text-[10px] font-black uppercase tracking-[.16em] text-slate-500">
        {label}
        {required && <em className="ml-1 not-italic text-rose-500">*</em>}
      </span>
