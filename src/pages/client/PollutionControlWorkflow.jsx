...
export default function PollutionControlWorkflow({ service, onBack }) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([getPollutionConfiguration(), getPollutionRequests(), getPollutionLedger()])
      .then(async ([config, requestData, ledgerData]) => {
        ...
        const quoteData = await getPollutionQuote({ requestType: type, industryCategory: category, grossBlock: Number(restoredGrossBlock || 0) });
        if (!active) return;
      ...
      .catch(() => active && setError("Unable to load Pollution Control Returns."))
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      getPollutionQuote({ requestType, industryCategory, grossBlock: Number(grossBlock || 0) })
      .then(setQuote)
  ...
  const valid = Boolean(
    Number(grossBlock) > 0 && waterKld !== "" && Number(waterKld) >= 0 && emissionSource &&
  ...
  async function saveDraft() {
    try {
      ...
      setRequestId(data.request.id);
      setDraftCode(data.request.request_code);
      await Swal.fire({ icon: "success", title: "Pollution Control draft saved", text: `${data.request.request_code} can be resumed later.`, confirmButtonColor: "#2952ff" });
    } catch (e) {
  ...
  async function submit() {
    ...
    try {
      ...
      const [nextQuote, nextLedger] = await Promise.all([getPollutionQuote({ requestType, industryCategory, grossBlock: Number(grossBlock || 0) }), getPollutionLedger()]);
      setQuote(nextQuote);
      ...
      await Swal.fire({ icon: "success", title: "Pollution Control request submitted", text: `${data.request.request_code} has been sent to the admin team.`, confirmButtonColor: "#2952ff" });
      onBack();
    } catch (e) {
      if (e.response?.status === 402) {
        ...
        Swal.fire("Unable to submit Pollution Control request", e.response?.data?.message || "Please try again.", "error");
      }
    } finally { setBusy(false); }
  }
  ...
  return (
    <div className="min-h-[calc(100vh-7rem)]">
      <div className="mx-auto max-w-7xl px-4 py-4 lg:px-6">
        ...
        {draftCode && <div className="mb-3 flex gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-3 text-blue-900"><Save size={17} /><div><b className="text-xs">Draft {draftCode} restored</b><p className="text-[11px] text-blue-600">Continue your Pollution Control filing.</p></div></div>}
          ...
          <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1.45fr)_360px]">
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2 rounded-2xl border border-slate-200 p-2 md:grid-cols-4">
                {(config?.industryCategories || []).map((category) => <button key={category.id} onClick={() => setIndustryCategory(category.id)} className={`rounded-xl px-3 py-3 text-left ${industryCategory === category.id ? "bg-[#2952ff] text-white shadow-sm" : "bg-slate-50 text-slate-500"}`}><b className="block text-xs uppercase">{category.label}</b><span className="text-[9px] uppercase opacity-70">Pollution category</span></button>)}
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="Gross Block Investment (INR)"><input type="number" min="1" value={grossBlock} onChange={(e) => setGrossBlock(e.target.value)} placeholder="Total plant and machinery investment" /></Field>
                <Field label="Water Consumption (KLD)"><input type="number" min="0" value={waterKld} onChange={(e) => setWaterKld(e.target.value)} placeholder="Daily water consumption" /></Field>
                <div className="md:col-span-2"><Field label="Emission Source"><select value={emissionSource} onChange={(e) => setEmissionSource(e.target.value)}><option value="">Select emission source</option>{(config?.emissionSources || []).map((source) => <option key={source.value} value={source.value}>{source.label}</option>)}</select></Field></div>
              </div>
              <div>
                <h2 className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em]"><Upload size={14} /> Required Pollution Control evidence</h2>
            ...
            <aside className={`overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl xl:sticky xl:top-24 xl:self-start ${quoteLoading ? "opacity-80" : ""}`}>
              ...
              <div className="space-y-5 p-5"><Ledger title="Prepaid Wallet" balance={costs.openingWalletBalance} lines={[["Pollution Control Official Fee", costs.officialFee]]} after={costs.closingWalletBalance} /><Ledger title="Corporate Credit Line" balance={costs.currentCreditLimit} lines={[["Technical Drafting & Site Audit", costs.draftingFee], ["Environmental Shield Premium", costs.shieldPremium], ["GST", costs.gst]]} after={costs.availableCreditAfter} /></div>
              <div className="flex justify-between bg-[#2952ff] p-5"><div><span className="text-[9px] font-black uppercase text-blue-100">Total Pollution Control Debit</span><b className="block text-2xl">₹{Number(costs.total || 0).toLocaleString("en-IN")}</b></div><span className="text-right text-[9px] uppercase text-white/70">SLA: {config?.sla}<br />{ledger.transactions.length} ledger entries</span></div>
            </aside>
          </div>
