...
import {
  ...
  submitEpcg,
  uploadEpcgDocument,
} from "../../services/epcgService";

const emptyFile = { status: "Not Uploaded", name: null };
export default function EpcgWorkflow({ service, onBack }) {
  ...
  const [epcgType, setEpcgType] = useState("new");
  const [raOffice, setRaOffice] = useState("");
  ...
  useEffect(() => {
    let active = true;
    Promise.all([getEpcgConfiguration(), getEpcgRequests(), getEpcgLedger()])
      .then(async ([config, requestData, ledgerData]) => {
        const draft = requestData.requests?.find(
          (item) => item.status === "DRAFT",
        );
        const body = draft?.payload || {};
        const mode = body.epcgType || "new";
        const quoteData = await getEpcgQuote(mode, body.dutyValue || 0);
        if (!active) return;
        ...
        setRaOffice(body.raOffice || "");
        setLicenseNumber(body.licenseNumber || "");
        setDutyValue(body.dutyValue || "");
        setFiles(restored);
      })
      .catch(() => active && setError("Unable to load EPCG Scheme Management."))
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      getEpcgQuote(epcgType, dutyValue || 0)
        .then(setQuote)
        .catch(() => setError("Unable to calculate EPCG pricing."))
        .finally(() => setQuoteLoading(false));
    }, 250);
    return () => clearTimeout(timer);
  }, [epcgType, dutyValue, initializing]);
  const config = configuration?.service;
  const documents = useMemo(
    () =>
      (config?.documents || []).filter(
        (item) => !item.epcgTypes || item.epcgTypes.includes(epcgType),
      ),
    [config, epcgType],
  );
  const payload = () => ({
    ...
    raOffice,
    licenseNumber,
    dutyValue: Number(dutyValue || 0),
    documents: files,
  });
  const valid = Boolean(
    raOffice &&
    (!["extension", "redemption"].includes(epcgType) || licenseNumber) &&
    (!["new", "redemption"].includes(epcgType) || Number(dutyValue) > 0) &&
    documents
  ...
  async function ensureDraft() {
    ...
    const data = await saveEpcgDraft(payload());
  ...
  async function selectFile(key, event) {
    ...
    try {
      ...
      const data = await uploadEpcgDocument(id, key, file);
      setFiles((current) => ({
        ...current,
        [key]: {
          status: "Uploaded",
  ...
  async function removeFile(key) {
    ...
    try {
      setUploading(key);
      await removeEpcgDocument(requestId, key);
  ...
  async function submit() {
    ...
    try {
      setBusy(true);
      const data = await submitEpcg(payload());
          ...
          <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1.45fr)_360px]">
            <div className="space-y-5">
              <div className="grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-1">
                {(config?.epcgTypes || []).map((mode) => (
                  <button
                    key={mode.id}
                    onClick={() => setEpcgType(mode.id)}
                    className={`rounded-xl px-3 py-3 ${epcgType === mode.id ? "border border-blue-200 bg-white text-[#2952ff] shadow-sm" : "text-slate-400"}`}
                  >
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="DGFT RA Office">
                  <select
                    value={raOffice}
                    onChange={(e) => setRaOffice(e.target.value)}
                  >
                    <option value="">Select RA office</option>
                    {(config?.raOffices || []).map((office) => (
                ...
                {["extension", "redemption"].includes(epcgType) && (
                  <Field label="EPCG Licence Number">
                    <input
                      value={licenseNumber}
                      onChange={(e) => setLicenseNumber(e.target.value)}
                      placeholder="E.g. 0330001234"
                    />
                  </Field>
                )}
                {["new", "redemption"].includes(epcgType) && (
                  <Field
                    label={
                      epcgType === "new"
                        ? "Estimated Duty Saved on Machinery"
                        : "Actual Duty Regularized"
                    }
                  >
                    <input
                      type="number"
              ...
              <div>
                <h2 className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em]">
                  <Upload size={14} /> Required statutory evidence
                </h2>
            ...
            >
              ...
              <div className="space-y-5 p-5">
                <Ledger
                  ...
                  lines={[["DGFT Official Fee", costs.officialFee]]}
                ...
                <Ledger
                  ...
                  lines={[
                    ["Application Drafting", costs.draftingFee],
                    ["Success Fee", costs.successFee],
                    ["GST", costs.gst],
                  ]}
