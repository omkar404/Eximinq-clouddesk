...
import {
  ...
  submitRex,
  uploadRexDocument,
} from "../../services/rexRegistrationService";
...
export default function RexRegistrationWorkflow({ service, onBack }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const config = await getRexConfiguration();
        const [requestsResult, ledgerResult] = await Promise.allSettled([
          getRexRequests(),
          getRexLedger(),
        ]);
        const data =
        ...
        let pricing = {};
        try {
          pricing = await getRexQuote(type);
        } catch {
          pricing = {};
        }
        if (!active) return;
        ...
        setRequestType(type);
        setHsCode(payload.hsCode || "");
        setValueAddition(payload.valueAddition ?? "");
        setDestination(payload.destination || "");
        setFiles(restored);
        setError("");
      } catch {
        if (active) setError("Unable to load the REX Registration workflow.");
      } finally {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      () =>
        getRexQuote(requestType)
          .then((data) => {
            setQuote(data);
            setError("");
          })
          .catch(() => setError("Unable to calculate REX pricing.")),
      200,
    );
    return () => clearTimeout(timer);
  }, [requestType, initializing, configuration]);
  ...
  const payload = () => ({
    ...
    requestType,
    hsCode,
    valueAddition: valueAddition === "" ? "" : Number(valueAddition),
    destination,
    documents: files,
  });
  ...
  const valid =
    /^\d{4,8}$/.test(hsCode) &&
    valueAddition !== "" &&
    valueAdditionNumber >= Number(config?.minimumValueAddition || 40) &&
    valueAdditionNumber <= 100 &&
    Boolean(destination) &&
    documents
  ...
  async function ensureDraft() {
    ...
    const data = await saveRexDraft(payload());
  ...
  async function selectFile(key, event) {
    ...
    try {
      ...
      const data = await uploadRexDocument(id, key, file);
      setFiles((current) => ({
        ...current,
        [key]: {
          status: "Uploaded",
  ...
  async function removeFile(key) {
    try {
      setUploading(key);
      await removeRexDocument(requestId, key);
  ...
  async function submit() {
    ...
    try {
      ...
      const data = await submitRex({ ...payload(), requestId: id });
          ...
          <div className="mb-3 flex gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-3 text-blue-900">
            ...
            <div>
              ...
              <p className="text-[11px] text-blue-600">
                Continue your REX registration.
              </p>
            </div>
          </div>
        )}
        <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_60px_rgba(15,23,42,.08)]">
          ...
          <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1.45fr)_360px]">
            <div className="space-y-5">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="HS Code (4–8 digits)">
                  <input
                    inputMode="numeric"
                    maxLength={8}
                    value={hsCode}
              ...
              {valueAddition !== "" &&
                valueAdditionNumber <
                  Number(config?.minimumValueAddition || 40) && (
                  <div className="flex gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-rose-800">
                    <AlertTriangle size={18} />
                    <p className="text-[11px] font-semibold">
                      Value addition must be at least{" "}
                      {config?.minimumValueAddition || 40}% to qualify under the
                      REX origin rules.
                    </p>
                  </div>
                )}
              <div>
                <h2 className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em]">
                  <Upload size={14} /> Required origin evidence
                </h2>
                  ...
                  {documents.map((document) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        >
                          {files[document.id]?.name || "Evidence file required"}
                        </span>
                      </div>
                      {files[document.id]?.status === "Uploaded" && (
                        <button
                      ...
                      <label className="cursor-pointer rounded-lg border bg-slate-50 px-4 py-2 text-[10px] font-black uppercase">
                        ...
                        <input
                          ...
                          onChange={(event) => selectFile(document.id, event)}
                        />
                      </label>
                    </article>
                  ))}
              ...
              <div className="flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
                <AlertTriangle size={18} />
                <p className="text-[11px] font-semibold">
                  Origin calculations and supporting inputs must be complete and
                  verifiable. Incorrect self-certification may affect
            ...
            <aside className="overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl xl:sticky xl:top-24 xl:self-start">
              ...
              <div className="space-y-5 p-5">
                <Ledger
                  ...
                  lines={[["Official Fee", quote.baseOfficial]]}
                ...
                <Ledger
                  ...
                  lines={[
                    ["Application Drafting", quote.draftingFee],
                    ["Origin Compliance Premium", quote.originPremium],
                    ["GST", quote.gst],
                  ]}
              ...
              <div className="flex justify-between bg-[#2952ff] p-5">
                <div>
                  ...
                  <b className="block text-2xl">₹{money(quote.total)}</b>
                </div>
          ...
          <footer className="flex flex-col justify-between gap-3 border-t bg-slate-50 p-5 sm:flex-row">
            ...
            <div className="flex gap-3">
              ...
              >
                <FileCheck2 size={16} />
                {valid ? "Submit REX Request" : "Incomplete Application"}
                <ArrowRight size={16} />
              </button>
            </div>
          </footer>
      ...
      <div className="space-y-2 text-xs">
        ...
        {lines.map(([label, amount]) => (
          ...
          >
            ...
            <b className="text-rose-400">- ₹{money(amount)}</b>
          </div>
        ))}
