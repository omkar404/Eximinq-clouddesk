...
export default function ClientOperations({ module }) {
  ...
  return <div className="min-h-full bg-[#f3f6fb] p-3 sm:p-5 lg:p-7"><div className="mx-auto max-w-[1600px] space-y-5">
    <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,.07)]">
      <div className="border-b-4 border-[#183b70] bg-[linear-gradient(115deg,#071d3e,#123b72_62%,#1d5d9c)] px-5 py-6 text-white sm:px-7 lg:px-9">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center"><div className="flex items-start gap-4"><div className="rounded-2xl border border-white/20 bg-white/10 p-3.5"><Icon size={26} /></div><div><p className="text-[10px] font-black uppercase tracking-[.24em] text-blue-200">{definition.eyebrow}</p><h1 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">{definition.title}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-blue-100">{definition.description}</p></div></div><div className="flex items-center gap-3"><span className="text-right text-[10px] font-bold uppercase tracking-wider text-blue-200">Auto-synced<br />{updatedAt ? new Date(updatedAt).toLocaleTimeString("en-IN") : "Waiting"}</span><button onClick={() => load(true)} disabled={refreshing} className="rounded-xl border border-white/20 bg-white/10 p-3 hover:bg-white/20"><RefreshCw size={18} className={refreshing ? "animate-spin" : ""} /></button><button onClick={exportCsv} className="flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-xs font-black text-[#123b72]"><Download size={15} /> Export CSV</button></div></div>
      </div>
      <div className="grid gap-px bg-slate-200 sm:grid-cols-2 xl:grid-cols-4">{metrics.map((metric) => <article key={metric.label} className="bg-white p-5"><div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-slate-400">{metric.label}</p><strong className="mt-2 block text-2xl font-black text-slate-900">{metric.value}</strong><span className="mt-1 block text-xs text-slate-500">{metric.note}</span></div><metric.icon size={20} className="text-[#245ea8]" /></div></article>)}</div>
    </section>
...
const Table = ({ headers, children }) => <table className="w-full min-w-[1050px] table-fixed text-left text-xs"><thead className="sticky top-0 z-10 bg-[#eef3f9] text-[10px] uppercase tracking-[.13em] text-slate-500"><tr>{headers.map((header) => <th key={header.label} className={`${header.width} px-5 py-4`}>{header.label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{children}</tbody></table>;
...
function WorkflowTable({ rows, onSelect }) { return <Table headers={[{label:"Request",width:"w-44"},{label:"Service",width:"w-64"},{label:"Submitted",width:"w-36"},{label:"Documents",width:"w-28"},{label:"Assigned Officer",width:"w-44"},{label:"Due Date",width:"w-36"},{label:"Status",width:"w-44"}]}>{rows.length ? rows.map((item) => <tr key={item.id} onClick={() => onSelect(item)} className="cursor-pointer hover:bg-blue-50/50"><td className="px-5 py-4 font-black text-[#195eaa]">{item.requestCode}</td><td className="px-5 py-4"><b>{item.serviceName}</b><span className="mt-1 block capitalize text-slate-400">{item.category}</span></td><td className="px-5 py-4">{date(item.submittedAt)}</td><td className="px-5 py-4 font-bold">{item.documentCount} files</td><td className="px-5 py-4">{item.assignment?.agentName || "Awaiting assignment"}</td><td className="px-5 py-4">{date(item.assignment?.dueDate)}</td><td className="px-5 py-4"><Status value={item.status} /></td></tr>) : <EmptyState message="No workflow records match the selected filters." />}</Table>; }
