...
const STEP_DEFINITIONS = [
  {
    ...
    description:
      "Capture the firm identity, core registrations, and GSTIN records.",
    icon: Building2,
  },
  {
    key: "branch-details",
    ...
    description:
      "Map branch-level details to the GSTINs already captured above.",
    icon: GitBranch,
  },
  {
...
function normalizeProfile(data) {
  ...
  return {
    ...
    partnershipDeedDocumentPath: source.partnership_deed_document_path || "",
...
function getSetupRouteValidationMessage(form, pendingDocuments = {}) {
  ...
  const hasCompleteKeyPerson = form.keyPeople.some(
    (person) => person.directorName.trim() && person.designation.trim(),
  );
  ...
  const invalidKeyPerson = form.keyPeople.find(
    (person) =>
  ...
  const firstKeyPerson = form.keyPeople.find((person) =>
    person.directorName.trim(),
  );
  const firstKeyPersonIndex = form.keyPeople.indexOf(firstKeyPerson);
  if (
    firstKeyPerson &&
    ((!firstKeyPerson.panDocumentPath &&
      !pendingDocuments[`key_person_pan_${firstKeyPersonIndex}`]?.token) ||
      (!firstKeyPerson.aadharDocumentPath &&
        !pendingDocuments[`key_person_aadhar_${firstKeyPersonIndex}`]?.token))
  ) {
    return "Upload both PAN and Aadhaar copies for the primary key person.";
  }

  const mandatoryPortals = [
    form.portalCredentials[0],
    form.portalCredentials[1],
  ];
  ...
  const hasIncompleteAdditionalPortal = (form.portalCredentials || [])
    ...
    .some(
      (portal) =>
        [portal.portalName, portal.userId, portal.password].some((value) =>
          value?.trim(),
        ) &&
  ...
  return {
    ...
    "branch-details": form.branches.filter(
      (item) =>
        item.gstin ||
        item.state ||
        item.branchAddresses?.some(
          (addressItem) => addressItem.branchName || addressItem.address,
        ),
    ).length,
...
function getStepValidationMessage(form, stepKey) {
  if (stepKey === "company-profile") {
    ...
    if (!/^[A-Z0-9]{10}$/.test(form.iecNumber.trim()))
      return "Enter a valid 10-character IEC number.";
    if (!/^\d{10}$/.test(form.firmMobileNo.trim()))
      return "Enter a valid 10-digit firm mobile number.";
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.firmEmail.trim()))
      return "Enter a valid firm email address.";
    const invalidGstin = form.gstinDetails.find(
      (item) =>
        item.gstin.trim() &&
        !/^\d{2}[A-Z]{5}\d{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/.test(item.gstin.trim()),
    );
    if (invalidGstin)
      return "Enter every GSTIN in the valid 15-character format.";
  }

  if (stepKey === "branch-details") {
    const hasBranch = form.branches.some((item) => {
      const linkedGstin = form.gstinDetails.find(
        (gstinItem) => gstinItem.gstin === item.gstin,
      );
      ...
      return (
        ...
        linkedAddresses.some(
          (addressItem) =>
            addressItem.branchName.trim() && addressItem.address.trim(),
        )
      );
    });
    if (!hasBranch) {
  ...
  if (stepKey === "key-person-details") {
    ...
    const invalidPerson = form.keyPeople.find(
      (item) =>
        item.directorName.trim() &&
    ...
    if (invalidPerson)
      return "Enter a valid 10-digit mobile, PAN, and 12-digit Aadhaar number for every key person.";
  }
  ...
  if (stepKey === "authorised-signatory-details") {
    ...
    const invalidSignatory = form.authorisedSignatories.find(
      (item) =>
        item.panName.trim() &&
        (!/^\d{10}$/.test(item.mobileNumber.trim()) ||
          !/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(item.panCardNo.trim()) ||
    ...
    if (invalidSignatory)
      return "Enter a valid 10-digit mobile, PAN, and 12-digit Aadhaar number for every authorised signatory.";
  }

  if (stepKey === "portal-credentials") {
    const hasCompletePortal = form.portalCredentials.some(
      (item) =>
        item.portalName?.trim() && item.userId?.trim() && item.password?.trim(),
    );
    if (!hasCompletePortal)
      return "Add at least one complete portal credential before saving the profile.";
    const incompletePortal = form.portalCredentials.find(
      (item) =>
        [item.portalName, item.userId, item.password].some((value) =>
          value?.trim(),
        ) &&
        (!item.portalName?.trim() ||
          !item.userId?.trim() ||
          !item.password?.trim()),
    );
    if (incompletePortal)
      return "Complete the portal name, user ID, and password for every portal row, or remove the incomplete row.";
  }
        ...
        <div className="mt-4 overflow-hidden rounded-[24px] border border-slate-200 bg-white">
          ...
          {!isPdfPreview && !isImagePreview ? (
            <div className="px-4 py-6 text-sm text-slate-500">
              Preview is not available for this file type, but the uploaded file
              can still be opened.
            </div>
          ) : null}
        </div>
...
function StepBadge({ index, step, isActive, isComplete, onClick }) {
  ...
  return (
    ...
    >
      ...
      <p
        className={`mt-2 text-sm leading-6 ${isActive ? "text-blue-100" : "text-slate-500"}`}
      >
        {step.description}
      </p>
...
export default function ClientCompanyProfile({ adminClientId = "" }) {
    ...
    const loadProfile = async () => {
      try {
        ...
        const normalized = ensureSetupRouteDefaults(
          normalizeProfile({
            ...response.data,
            user: isAdminManaged ? response.data.user : user,
          }),
        );
        const draft = isAdminManaged ? null : readDraft();
        const canEditProfile =
          response.data.onboarding?.companyProfileEditable !== false;
        const mergedProfile =
          canEditProfile && !isAdminManaged
  ...
  const activeCompanyProfileSubStep =
    COMPANY_PROFILE_SUBSTEPS[companyProfileSubStep];
  const activeQuickFormSection = QUICK_FORM_SECTIONS[quickFormSectionIndex];
  const isSetupRoute =
    !isAdminManaged && location.pathname === "/client/company-profile-setup";
  const isEditable =
    isAdminManaged ||
    (isSetupRoute && onboarding?.companyProfileEditable !== false);
  ...
  const savedDocumentUrls = useMemo(
    () =>
      Object.fromEntries(
        (documentCatalog || []).map((item) => [item.key, item.download_url]),
      ),
    [documentCatalog],
  );
  ...
  const updateGstinBranchAddressItem = (
    gstinIndex,
    addressIndex,
    field,
    value,
  ...
  const getAvailableGstinOptions = (currentBranchIndex) => {
    ...
    return gstinOptions.filter(
      (gstin) =>
        gstin === currentSelection || !selectedByOtherBranches.includes(gstin),
    );
  };

  const getLinkedGstinRecord = (gstinValue) =>
    form.gstinDetails.find((gstinItem) => gstinItem.gstin === gstinValue) ||
    null;
      ...
      branches: prev.branches.map((branch, currentBranchIndex) =>
        currentBranchIndex === branchIndex
          ? {
              ...branch,
              gstin: nextGstin,
              branchAddresses: linkedAddresses.map(
  ...
  const handleDocumentUpload = async ({ fieldKey, documentType, file }) => {
    ...
    try {
      ...
      const response = isAdminManaged
        ? await uploadAdminCompanyDocument(
            adminClientId,
            documentType,
            file,
            fieldKey,
  ...
  const handlePendingDocumentRemove = async ({ fieldKey, documentType }) => {
    ...
    try {
      if (isAdminManaged) {
        await removeAdminTempCompanyDocument(
          adminClientId,
          documentType,
          pendingFile.token,
        );
      ...
      Swal.fire({
        ...
        text:
          err.response?.data?.message || "Please try removing this file again.",
        confirmButtonColor: "#101eb9",
      });
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (isSetupRoute) {
      const validationMessage = getSetupRouteValidationMessage(
        form,
        pendingDocuments,
      );

      if (validationMessage) {
        const sectionIndex = /company|firm mobile|email/i.test(
          validationMessage,
        )
          ? 0
    ...
    try {
      ...
      Object.values(pendingDocuments).forEach((item) =>
      ...
      Swal.fire({
        ...
        title: "Save failed",
        text:
          err.response?.data?.message || "Unable to save the company profile.",
        confirmButtonColor: "#101eb9",
      });
    } finally {
      setSaving(false);
  ...
  if (isSetupRoute) {
    ...
    const uploadedSetupDocumentCount = [
      ...
      form.shopEstablishmentDocumentPath ||
        pendingDocuments.shopEstablishment?.token,
      form.partnershipDeedDocumentPath ||
        pendingDocuments.partnershipDeed?.token,
    ].filter(Boolean).length;
      ...
      <div className="mx-auto max-w-6xl space-y-6 pb-10">
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
                  Complete the required company setup form below. Once
                  submitted, the request will be shared with the admin for
                  review before dashboard access is enabled.
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <SummaryCard
                  title="CDCR Number"
                  value={user?.userCode || form.cdcrNo}
                />
                <SummaryCard
                  title="Key People"
                  value={String(form.keyPeople.length)}
                />
                <SummaryCard
                  title="Uploaded Docs"
                  value={String(uploadedSetupDocumentCount)}
                />
              </div>
            </div>
          </div>
              ...
              <div className="mt-8 space-y-6">
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <SelectField
                    ...
                    onChange={(event) =>
                      updateField("concernNature", event.target.value)
                    }
                ...
                <div className="rounded-[28px] border border-slate-200 bg-[linear-gradient(135deg,#fbfdff_0%,#f8fbff_100%)] p-5">
                  ...
                  <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    <SelectField
                      ...
                      onChange={(event) =>
                        updateField("udhyamStatus", event.target.value)
                      }
                  ...
                  >
                    ...
                    <div className="mt-5 grid gap-4 lg:grid-cols-2">
                      <DocumentField
                        ...
                        savedDocumentUrl={
                          savedDocumentUrls[`key_person_pan_${index}`]
                        }
                        ...
                        onUpload={handleDocumentUpload}
                        uploading={Boolean(
                          uploadingFields[`key_person_pan_${index}`],
                        )}
                        onRemove={handlePendingDocumentRemove}
                        onViewSavedFile={viewSavedDocument}
                        readOnly={!isEditable}
                      ...
                      <DocumentField
                        ...
                        savedDocumentUrl={
                          savedDocumentUrls[`key_person_aadhar_${index}`]
                        }
                        ...
                        onUpload={handleDocumentUpload}
                        uploading={Boolean(
                          uploadingFields[`key_person_aadhar_${index}`],
                        )}
                        onRemove={handlePendingDocumentRemove}
                        onViewSavedFile={viewSavedDocument}
                        readOnly={!isEditable}
              ...
              <div className="mt-8 space-y-5">
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            removeListItem("portalCredentials", actualIndex)
                          }
                          disabled={!isEditable}
                          className="inline-flex items-center gap-2 rounded-2xl border border-rose-200 bg-white px-4 py-2 text-sm font-semibold text-rose-600"
                        >
                ...
                {isEditable ? (
                  <button
                    type="button"
                    onClick={() =>
                      addListItem("portalCredentials", emptyPortal)
                    }
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <button
            type="button"
            onClick={() =>
              setQuickFormSectionIndex((index) => Math.max(index - 1, 0))
            }
      ...
      >
        <div className="border-b border-slate-200/80 px-6 py-6 lg:px-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div
              className={`grid gap-3 ${isSetupRoute ? "sm:grid-cols-2 lg:grid-cols-3" : "sm:grid-cols-3"}`}
            >
              <SummaryCard
                title="CDCR Number"
                value={user?.userCode || form.cdcrNo}
              ...
              <SummaryCard
                title="Sections Done"
                value={`${completedSteps}/5`}
              />
              <SummaryCard
                title="GSTIN Entries"
                value={String(gstinOptions.length || 0)}
              />
            </div>
          </div>
        </div>
        ...
        <div
          className={`overflow-x-auto px-6 py-5 lg:px-8 ${isSetupRoute ? "bg-white/35" : ""}`}
        >
                    ...
                    {companyProfileSubStep === 0 ? (
                      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                        ...
                        <SelectField
                          ...
                          value={form.isSez}
                          onChange={(event) =>
                            updateField("isSez", event.target.value)
                          }
                        ...
                        <TextareaField
                          ...
                          onChange={(event) =>
                            updateField("headOfficeAddress", event.target.value)
                          }
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              addListItem("gstinDetails", emptyGstin)
                            }
                            disabled={!isEditable}
                            ...
                            >
                              <div className="grid gap-4 md:grid-cols-[1fr,auto]">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    removeListItem("gstinDetails", index)
                                  }
                                  disabled={
                                    !isEditable ||
                                    form.gstinDetails.length === 1
                                  }
                              ...
                              <div className="mt-4">
                                <DocumentField
                                  ...
                                  savedDocumentUrl={
                                    savedDocumentUrls[`gstin_${index}`]
                                  }
                                ...
                                <div className="flex items-center justify-between gap-4">
                                  <div>
                                    ...
                                    <p className="mt-1 text-sm text-slate-500">
                                      Add multiple branch addresses under this
                                      GSTIN.
                                    </p>
                                  </div>
                                  <button
                      ...
                      <button
                        ...
                        onClick={() =>
                          moveCompanyProfileSubStep(
                            Math.max(companyProfileSubStep - 1, 0),
                          )
                        }
                      ...
                      <button
                        ...
                        onClick={() =>
                          moveCompanyProfileSubStep(
                            Math.min(
                              companyProfileSubStep + 1,
                              COMPANY_PROFILE_SUBSTEPS.length - 1,
                            ),
                          )
                        }
                        disabled={
                          companyProfileSubStep ===
                          COMPANY_PROFILE_SUBSTEPS.length - 1
                        }
                    ...
                    >
                      ...
                      <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                        <SelectField
                          ...
                          value={branch.gstin}
                          onChange={(event) =>
                            handleBranchGstinChange(index, event.target.value)
                          }
                      ...
                      <div className="mt-6 space-y-4">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-500">
                            These addresses are fetched automatically from the
                            selected GSTIN in Company Profile.
                          </p>
                        </div>

                        {getLinkedBranchAddresses(branch.gstin).length > 0 ? (
                          getLinkedBranchAddresses(branch.gstin).map(
                            (addressItem, addressIndex) => (
                              <div
                                key={
                                  addressItem.id ||
                                  `${branch.id}-linked-${addressIndex}`
                                }
                                className="rounded-[24px] border border-slate-200 bg-white p-4"
                              >
                                <div className="mb-4 flex items-center justify-between gap-4">
                          ...
                          <div className="rounded-[24px] border border-slate-200 bg-white p-4">
                            <p className="text-sm text-slate-500">
                              No branch addresses are available under this GSTIN
                              yet. Add them first in Company Profile - GSTIN
                              Details.
                            </p>
                          </div>
                        ) : (
                          <div className="rounded-[24px] border border-dashed border-slate-200 bg-white p-4">
                            <p className="text-sm text-slate-500">
                              Select a linked GSTIN to load its branch addresses
                              here.
                            </p>
                          </div>
                        )}
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            removeListItem("authorisedSignatories", index)
                          }
                          disabled={
                            !isEditable ||
                            form.authorisedSignatories.length === 1
                          }
                          className="inline-flex items-center gap-2 rounded-2xl border border-rose-200 px-4 py-3 text-sm font-semibold text-rose-600 disabled:cursor-not-allowed disabled:opacity-40"
                        >
                          <Trash2 size={16} />
                      ...
                      <div className="mt-6 grid gap-4 lg:grid-cols-2">
                        <DocumentField
                          ...
                          savedDocumentUrl={
                            savedDocumentUrls[
                              `authorised_signatory_pan_${index}`
                            ]
                          }
                        ...
                        <DocumentField
                          ...
                          savedDocumentUrl={
                            savedDocumentUrls[
                              `authorised_signatory_aadhar_${index}`
                            ]
                          }
                        ...
                        >
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                removeListItem("portalCredentials", index)
                              }
                              disabled={
                                !isEditable ||
                                form.portalCredentials.length === 1
                              }
                              className="inline-flex items-center gap-2 rounded-2xl border border-rose-200 px-4 py-2 text-sm font-semibold text-rose-600 disabled:cursor-not-allowed disabled:opacity-40"
                            >
                              <Trash2 size={16} />
                  ...
                  <div className="rounded-[28px] border border-slate-200 bg-[linear-gradient(135deg,#f8fbff_0%,#ffffff_100%)] p-6">
                    ...
                    <div className="mt-5 grid gap-3 md:grid-cols-2">
                      <SummaryCard
                        ...
                        value={String(form.portalCredentials.length)}
                      />
                      <SummaryCard
                        title="Signatories"
                        value={String(form.authorisedSignatories.length)}
                      />
                      <SummaryCard
                        title="Key People"
                        value={String(form.keyPeople.length)}
                      ...
                      <SummaryCard
                        title="Branches"
                        value={String(form.branches.length)}
                      />
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  moveToStep(
                    Math.min(activeStep + 1, STEP_DEFINITIONS.length - 1),
                  );
                }}
