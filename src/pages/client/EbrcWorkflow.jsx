...
import {
  ...
  submitEbrc,
  uploadEbrcDocument,
} from "../../services/ebrcService";

const emptyFile = { status: "Not Uploaded", name: null };
export default function EbrcWorkflow({ service, onBack }) {
  ...
  const [ebrcType, setEbrcType] = useState("Auto");
  ...
  useEffect(() => {
    let active = true;
    Promise.all([getEbrcConfiguration(), getEbrcRequests(), getEbrcLedger()])
      .then(async ([config, data, ledgerData]) => {
        const draft = data.requests?.find((item) => item.status === "DRAFT");
        const payload = draft?.payload || {};
        const type = payload.ebrcType || "Auto";
        const pricing = await getEbrcQuote(type);
        if (!active) return;
        ...
        setEbrcType(type);
        setShippingBillNumber(payload.shippingBillNumber || "");
        setRealizedValue(payload.realizedValue ?? "");
        setIrmNumber(payload.irmNumber || "");
        setBankAdCode(payload.bankAdCode || "");
        setFiles(restored);
      })
      .catch(() => active && setError("Unable to load e-BRC service."))
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      () =>
        getEbrcQuote(ebrcType)
          .then((data) => {
            setQuote(data);
            setError("");
          })
          .catch(() => setError("Unable to calculate e-BRC pricing.")),
      250,
    );
    return () => clearTimeout(timer);
  }, [ebrcType, initializing]);
  const config = configuration?.service;
  const documents = useMemo(
    () =>
      (config?.documents || []).filter(
        (item) => !item.ebrcTypes || item.ebrcTypes.includes(ebrcType),
      ),
    [config, ebrcType],
  );
  const payload = () => ({
    ...
    ebrcType,
    shippingBillNumber,
    realizedValue: realizedValue === "" ? null : Number(realizedValue),
    irmNumber,
    bankAdCode,
    documents: files,
  });
  const valid = Boolean(
    /^\d{7}$/.test(shippingBillNumber) &&
    Number(realizedValue) > 0 &&
    (ebrcType !== "Reconcile" ||
      (irmNumber.trim() && /^\d{7}$/.test(bankAdCode))) &&
    documents
  ...
  async function ensureDraft() {
    ...
    const data = await saveEbrcDraft(payload());
  ...
  async function selectFile(key, event) {
    ...
    try {
      ...
      const data = await uploadEbrcDocument(id, key, file);
      setFiles((current) => ({
        ...current,
        [key]: {
          status: "Uploaded",
      ...
      Swal.fire(
        "Upload failed",
        e.response?.data?.message || "The evidence could not be uploaded.",
        "error",
      );
    } finally {
      setUploading("");
  ...
  async function removeFile(key) {
    try {
      setUploading(key);
      await removeEbrcDocument(requestId, key);
  ...
  async function saveDraft() {
    try {
      ...
      Swal.fire(
        ...
        e.response?.data?.message || "Please check the entered values.",
        "error",
      );
    } finally {
      setBusy(false);
  ...
  async function submit() {
    ...
    try {
      setBusy(true);
      const data = await submitEbrc(payload());
          ...
          <div className="mb-3 flex gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-3 text-blue-900">
            ...
            <div>
              ...
              <p className="text-[11px] text-blue-600">
                Continue your e-BRC declaration and reconciliation.
              </p>
            </div>
          </div>
        )}
        <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_60px_rgba(15,23,42,.08)]">
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-1 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.16em] text-slate-500">
                  {config?.standard}
                  <Zap size={11} className="text-amber-500" /> Priority
                  settlement
                </p>
              </div>
            </div>
          ...
          <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1.45fr)_360px]">
            <div className="space-y-5">
              <OptionGroup
                options={config?.issuanceMethods}
                value={ebrcType}
                onChange={setEbrcType}
              />
              <div className="flex gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-blue-900">
                <Building2 size={18} />
                <div>
                  <b className="text-[10px] uppercase tracking-widest">
                    Exporter identity
                  </b>
                  <p className="text-xs font-bold">
                    ...
                    {configuration?.clientContext?.iecNumber ||
                      "Pending in company profile"}
                  </p>
                </div>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Shipping Bill Number (7 digits)">
                  <input
                    inputMode="numeric"
                    maxLength={7}
                    value={shippingBillNumber}
                    onChange={(e) =>
                      setShippingBillNumber(e.target.value.replace(/\D/g, ""))
                    }
                    placeholder="E.g. 1234567"
                  />
                </Field>
                <Field label="Total Realized Value (FCY)">
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    placeholder="Foreign currency value"
                  />
                </Field>
                {ebrcType === "Reconcile" && (
                  <>
                    <Field label="IRM Number">
                      <input
                        value={irmNumber}
                        onChange={(e) =>
                          setIrmNumber(e.target.value.toUpperCase())
                        }
                        placeholder="Bank inward remittance reference"
                      />
                    </Field>
                    <Field label="Bank AD Code (7 digits)">
                      <input
                        inputMode="numeric"
                        maxLength={7}
                        value={bankAdCode}
                        onChange={(e) =>
                          setBankAdCode(e.target.value.replace(/\D/g, ""))
                        }
              ...
              <div>
                <h2 className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em]">
                  <Upload size={14} /> Required realization evidence
                </h2>
                  ...
                  {documents.map((doc) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        >
                          {files[doc.id]?.name || "Evidence not attached"}
                        </span>
                      </div>
                      {files[doc.id]?.status === "Uploaded" && (
                        <button
              ...
              <div className="flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
                ...
                <p className="text-[11px] font-semibold">
                  Realisation details must match the bank advice and export
                  documents. Incorrect declarations can affect EDPMS closure and
                  FEMA compliance.
                </p>
              </div>
            </div>
            <aside className="overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl xl:sticky xl:top-24 xl:self-start">
              ...
              <div className="space-y-5 p-5">
                <Ledger
                  ...
                  lines={[["Official Fee", quote.officialFee]]}
          ...
          <footer className="flex flex-col justify-between gap-3 border-t bg-slate-50 p-5 sm:flex-row">
            ...
            <div className="flex gap-3">
              ...
              >
                {valid ? "Submit e-BRC Request" : "Incomplete Evidence"}
                <ArrowRight size={16} />
              </button>
            </div>
          </footer>
    ...
    <div className="grid gap-2 rounded-2xl bg-slate-100 p-1 md:grid-cols-2">
      {options.map((item) => (
        <button
          ...
          className={`rounded-xl px-3 py-3 ${value === item.id ? "border border-blue-200 bg-white text-[#2952ff] shadow-sm" : "text-slate-400"}`}
        >
          <b className="block text-xs uppercase">{item.label}</b>
          <span className="text-[9px] uppercase">{item.description}</span>
        </button>
      ))}
    </div>
  );
      ...
      <div className="space-y-2 text-xs">
        ...
        {lines.map(([label, amount]) => (
          ...
          >
            ...
            <b
              className={
                Number(amount) > 0 ? "text-rose-400" : "text-slate-300"
              }
            >
