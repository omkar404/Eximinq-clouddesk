...
import {
  getAqcsPqmsConfiguration,
  getAqcsPqmsLedger,
  getAqcsPqmsQuote,
  getAqcsPqmsRequests,
  removeAqcsPqmsDocument,
  saveAqcsPqmsDraft,
  submitAqcsPqms,
  uploadAqcsPqmsDocument,
} from "../../services/aqcsPqmsService";

const emptyFile = { status: "Not Uploaded", name: null };
export default function AqcsPqmsWorkflow({ service, onBack }) {
  ...
  const [requestMode, setRequestMode] = useState("PQMS");
  const [requestType, setRequestType] = useState("Clearance");
  const [portCode, setPortCode] = useState("");
  const [consignmentValue, setConsignmentValue] = useState("");
  const [treatmentType, setTreatmentType] = useState("Fumigation");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([config, data, ledgerData]) => {
        const draft = data.requests?.find((x) => x.status === "DRAFT");
        const p = draft?.payload || {};
        const mode = p.requestMode || "PQMS",
          type = p.requestType || "Clearance",
          value = p.consignmentValue || "";
        const pricing = await getAqcsPqmsQuote(mode, type, value);
        if (!active) return;
        const restored = Object.fromEntries(
          (config.service.documents || []).map((x) => [x.id, { ...emptyFile }]),
        );
        ...
        setRequestMode(mode);
        setRequestType(type);
        setPortCode(p.portCode || "");
        setConsignmentValue(value);
        setTreatmentType(p.treatmentType || "Fumigation");
        setFiles(restored);
      })
      .catch(() => active && setError("Unable to load AQCS & PQMS."))
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      () =>
        getAqcsPqmsQuote(requestMode, requestType, consignmentValue)
          .then((data) => {
            setQuote(data);
            setError("");
          })
          .catch(() => setError("Unable to calculate quarantine pricing.")),
      250,
    );
    return () => clearTimeout(timer);
  }, [requestMode, requestType, consignmentValue, initializing]);
  const config = configuration?.service;
  const documents = useMemo(
    () =>
      (config?.documents || []).filter(
        (x) =>
          (!x.requestModes || x.requestModes.includes(requestMode)) &&
          (!x.requestTypes || x.requestTypes.includes(requestType)),
      ),
    [config, requestMode, requestType],
  );
  const payload = () => ({
    requestId,
    requestMode,
    requestType,
    portCode,
    consignmentValue: Number(consignmentValue || 0),
    treatmentType,
    documents: files,
  });
  const valid = Boolean(
    portCode.trim() &&
    Number(consignmentValue) > 0 &&
    documents
      .filter((x) => x.required)
      .every((x) => files[x.id]?.status === "Uploaded"),
  );
  async function ensureDraft() {
    if (requestId) return requestId;
    const data = await saveAqcsPqmsDraft(payload());
  ...
  async function selectFile(key, event) {
    ...
    try {
      ...
      const data = await uploadAqcsPqmsDocument(id, key, file);
      setFiles((c) => ({
        ...c,
        [key]: {
          status: "Uploaded",
  ...
  async function removeFile(key) {
    try {
      setUploading(key);
      await removeAqcsPqmsDocument(requestId, key);
      setFiles((c) => ({ ...c, [key]: { ...emptyFile } }));
    } catch (e) {
      Swal.fire(
  ...
  async function submit() {
    ...
    try {
      setBusy(true);
      const data = await submitAqcsPqms(payload());
      ...
      await Swal.fire({
        icon: "success",
        title: "Quarantine request submitted",
          ...
          <div className="mb-3 flex gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-3 text-blue-900">
            ...
            <div>
              ...
              <p className="text-[11px] text-blue-600">
                Continue your bio-security request.
              </p>
            </div>
          </div>
        )}
        <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_60px_rgba(15,23,42,.08)]">
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  <h1 className="text-xl font-black uppercase">
                    {requestMode
                      ? `${requestMode} Bio-Security Audit`
                      : service.title}
                  </h1>
                ...
                <p className="mt-1 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.16em] text-slate-500">
                  {config?.standard}
                  <Zap size={11} className="text-amber-500" /> Demurrage
                  priority
                </p>
              </div>
            </div>
          ...
          <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1.45fr)_360px]">
            <div className="space-y-5">
              <div className="grid gap-2 rounded-2xl bg-slate-100 p-1 md:grid-cols-2">
                {(config?.requestModes || []).map((mode) => (
                  <button
                    key={mode.id}
                    onClick={() => setRequestMode(mode.id)}
                    className={`rounded-xl px-3 py-3 ${requestMode === mode.id ? "border border-blue-200 bg-white text-[#2952ff] shadow-sm" : "text-slate-400"}`}
                  >
              ...
              <div className="grid gap-2 rounded-2xl bg-slate-100 p-1 md:grid-cols-2">
                {(config?.requestTypes || []).map((type) => (
                  <button
                    ...
                    className={`rounded-xl px-3 py-3 ${requestType === type.id ? "border border-blue-200 bg-white text-[#2952ff] shadow-sm" : "text-slate-400"}`}
                  >
                    <b className="block text-xs uppercase">{type.label}</b>
                    <span className="text-[9px] uppercase">
                      {type.description}
                    </span>
                  </button>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="Consignment Value (INR)">
                  <input
                    type="number"
                    min="0"
                ...
                <Field label="Phyto-treatment Method">
                  <select
                    value={treatmentType}
                    onChange={(e) => setTreatmentType(e.target.value)}
                  >
                    {(config?.treatmentMethods || []).map((x) => (
              ...
              <div className="flex gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-blue-900">
                <Anchor size={18} />
                <p className="text-[11px] font-semibold">
                  {requestMode === "AQCS"
                    ? "Health Certificate clauses will be checked against Indian sanitary protocols before portal filing."
                    : `Phytosanitary evidence and ${treatmentType} treatment dates will be cross-checked against shipment records.`}
                </p>
              </div>
              <div>
                <h2 className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em]">
                  <Upload size={14} /> Required bio-evidence
                </h2>
                  ...
                  {documents.map((doc) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <span
                          className={`block truncate text-[10px] ${files[doc.id]?.status === "Uploaded" ? "text-emerald-600" : "text-slate-400"}`}
                        >
                          {files[doc.id]?.name || "Compliance file required"}
                        </span>
                      </div>
                      {files[doc.id]?.status === "Uploaded" && (
                        <button
              ...
              <div className="flex gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-rose-800">
                ...
                <p className="text-[11px] font-semibold">
                  Certificates containing overwriting or undeclared stamps may
                  be rejected by the Bio-Security portal.
                </p>
              </div>
            </div>
            <aside className="overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl xl:sticky xl:top-24 xl:self-start">
              ...
              <div className="space-y-5 p-5">
                <Ledger
                  ...
                  lines={[
                    ["Statutory Testing & Portal Fee", quote.officialFee],
                  ]}
                ...
                <Ledger
                  ...
                  lines={[
                    ["Technical Dossier & Coordination", quote.draftingFee],
                    ["Bio-Security Shield Premium", quote.successFee],
                    ["GST (18%)", quote.gst],
                  ]}
          ...
          <footer className="flex flex-col justify-between gap-3 border-t bg-slate-50 p-5 sm:flex-row">
            ...
            <div className="flex gap-3">
              ...
              >
                {valid ? "Confirm & File Audit" : "Incomplete Audit"}
                <ArrowRight size={16} />
              </button>
            </div>
          </footer>
