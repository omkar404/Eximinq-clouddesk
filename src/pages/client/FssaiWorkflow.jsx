...
import { Activity, AlertTriangle, Anchor, ArrowLeft, ArrowRight, Calendar, Download, FileCheck2, FileText, History, Info, PlusCircle, Receipt, Save, Ship, Soup, Upload, Wallet, X, Zap } from "lucide-react";
import { downloadFssaiDocument, getFssaiConfiguration, getFssaiLedger, getFssaiQuote, getFssaiRequests, removeFssaiDocument, saveFssaiDraft, submitFssai, uploadFssaiDocument } from "../../services/fssaiService";

const emptyFile = { status: "Not Uploaded", name: null };
const money = (value) => Number(value || 0).toLocaleString("en-IN", { maximumFractionDigits: 2 });

export default function FssaiWorkflow({ onBack }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setShelfLifeRemaining(payload.shelfLifeRemaining ?? ""); setFiles(restored); setError("");
  ...
  const payload = () => ({ requestId, requestMode, licenseType, licenseRole, foodCategory, shelfLifeRemaining: shelfLifeRemaining === "" ? "" : Number(shelfLifeRemaining), documents: files });
  const requiredForMode = (item) => item.requiredModes ? item.requiredModes.includes(requestMode) : item.required;
  const valid = Boolean(foodCategory && (requestMode !== "FICS" || (Number(shelfLifeRemaining) >= 60 && Number(shelfLifeRemaining) <= 100)) && documents.filter(requiredForMode).every((item) => files[item.id]?.status === "Uploaded"));
  ...
  async function downloadFile(key) { try { await downloadFssaiDocument(requestId, key, files[key]?.name); } catch (e) { Swal.fire("Download failed", e.response?.data?.message || "The document could not be downloaded.", "error"); } }
  ...
  return <div className="min-h-[calc(100vh-7rem)]"><div className="mx-auto max-w-7xl px-4 py-4 lg:px-6">
    ...
    <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_60px_rgba(15,23,42,.08)]">
      <header className="flex justify-between border-b p-5"><div className="flex gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#2952ff] text-white"><Soup/></div><div><button onClick={onBack} className="flex items-center gap-2 text-sm font-bold text-slate-500"><ArrowLeft size={16}/> Back to Compliance</button><div className="mt-1 flex flex-wrap items-center gap-2"><h1 className="text-xl font-black uppercase">FSSAI Compliance Hub</h1><span className="rounded-full bg-slate-900 px-2.5 py-1 text-[9px] font-black uppercase tracking-widest text-white">FoSCoS Statutory</span></div><p className="mt-1 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.16em] text-slate-500">{draftCode||"New FSSAI Task"}<Zap size={11} className="text-amber-500"/> Priority Filing</p></div></div><button onClick={onBack}><X className="text-slate-400"/></button></header>
      <div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1.45fr)_360px]"><div className="space-y-5">
        <div className="grid grid-cols-2 gap-2 rounded-2xl border border-slate-100 bg-slate-50 p-1 sm:grid-cols-4">{(config?.requestModes || []).map((mode) => <button key={mode.id} onClick={() => setRequestMode(mode.id)} className={`rounded-xl px-3 py-3 ${requestMode === mode.id ? "bg-white text-[#2952ff] shadow-md" : "text-slate-400"}`}><b className="block text-[9px] uppercase">{mode.id==="Labelling"?"Labels":mode.label}</b><span className="text-[7px] font-bold uppercase">{{License:"New/Renew",Returns:"Annual",Labelling:"Scrubbing",FICS:"Clearance"}[mode.id]}</span></button>)}</div>
        {requestMode==="License"&&<div className="grid grid-cols-2 gap-2 rounded-2xl bg-slate-100/50 p-1"><button onClick={()=>setLicenseType("New")} className={`rounded-xl py-2 text-[9px] font-black uppercase ${licenseType==="New"?"bg-white text-slate-900 shadow-sm":"text-slate-400"}`}><PlusCircle size={10} className="mr-1 inline"/>New App</button><button onClick={()=>setLicenseType("Renewal")} className={`rounded-xl py-2 text-[9px] font-black uppercase ${licenseType==="Renewal"?"bg-white text-slate-900 shadow-sm":"text-slate-400"}`}><History size={10} className="mr-1 inline"/>Renewal</button></div>}
        {requestMode==="License"&&<div><p className="mb-3 text-[10px] font-black uppercase tracking-[.15em] text-slate-400">Kind of Business (KoB)</p><div className="grid grid-cols-2 gap-3"><RoleButton active={licenseRole==="Importer"} icon={<Anchor size={18}/>} label="Importer" onClick={()=>setLicenseRole("Importer")}/><RoleButton active={licenseRole==="Exporter"} icon={<Ship size={18}/>} label="Exporter" onClick={()=>setLicenseRole("Exporter")}/></div></div>}
        <div className="grid gap-4 md:grid-cols-2"><Field label="Food Category"><select value={foodCategory} onChange={(e) => setFoodCategory(e.target.value)}>{(config?.foodCategories || []).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></Field>{requestMode === "FICS" ? <Field label="Remaining Shelf Life (%)"><span className="relative block"><input type="number" min="0" max="100" value={shelfLifeRemaining} onChange={(e) => setShelfLifeRemaining(e.target.value)} placeholder="e.g. 75"/><span className="absolute right-4 top-3.5 text-xs font-black text-slate-400">%</span></span></Field> : <div><span className="mb-1.5 flex items-center gap-2 px-1 text-[10px] font-black uppercase tracking-[.16em] text-slate-500"><Activity size={12}/>FY Reporting Period</span><div className="flex items-center justify-between rounded-xl bg-slate-50 p-3.5 text-sm font-black text-slate-400"><span>{config?.reportingPeriod}</span><History size={14}/></div></div>}</div>
        <div><h2 className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em]"><Upload size={14}/> Required Audit Evidence</h2><div className="space-y-2">{documents.map((document) => {const required=requiredForMode(document);return <article key={document.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 p-3"><FileText className={files[document.id]?.status === "Uploaded" ? "text-emerald-500" : "text-slate-400"}/><div className="min-w-0 flex-1"><b className="block text-xs uppercase">{document.label}{required && <em className="ml-2 not-italic text-[9px] text-rose-500">REQUIRED</em>}</b><span className={`block truncate text-[10px] ${files[document.id]?.status === "Uploaded" ? "text-emerald-600" : "text-slate-400"}`}>{files[document.id]?.name || "Compliance file required"}</span></div>{files[document.id]?.status === "Uploaded" && <button onClick={() => downloadFile(document.id)} title="Download uploaded document" className="text-[#2952ff]"><Download size={16}/></button>}{files[document.id]?.status === "Uploaded" && <button onClick={() => removeFile(document.id)} className="text-[10px] font-black uppercase text-rose-500">Remove</button>}<label className="cursor-pointer rounded-lg border bg-slate-50 px-4 py-2 text-[10px] font-black uppercase">{uploading === document.id ? "Uploading..." : files[document.id]?.status === "Uploaded" ? "Replace" : "Upload"}<input className="sr-only" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" onChange={(event) => selectFile(document.id, event)}/></label></article>})}</div></div>
...
function RoleButton({active,icon,label,onClick}){return <button onClick={onClick} className={`flex flex-col items-center gap-2 rounded-2xl border p-4 text-[10px] font-black uppercase ${active?"border-[#2952ff] bg-blue-50 text-[#2952ff] shadow-sm":"border-slate-100 text-slate-400"}`}>{icon}{label}</button>}
