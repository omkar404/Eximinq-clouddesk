  ...
  if (selected) return <div className="min-h-full bg-slate-50 p-5 lg:p-8"><input ref={input} type="file" className="hidden" onChange={upload} />
    <div className="mx-auto max-w-6xl"><button onClick={() => setSelected(null)} className="mb-5 flex items-center gap-2 font-bold text-slate-500"><ArrowLeft /> My tasks</button>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]"><div className="space-y-6"><Card title="Assignment" icon={<BriefcaseBusiness />}>
        ...
        <aside className="space-y-6"><Card title="Source documents" icon={<FileText />}><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">{(selected.documents || []).map((file) =>
          <button key={`original-${file.id}`} onClick={() => downloadWorkflowFile({ role: "agent", requestId: selected.id, kind: "original", fileId: file.id, name: file.name })} className="compact-document-card text-left"><span className="compact-document-icon"><FileText size={16}/></span><span className="min-w-0 flex-1"><strong>{file.name}</strong><small>Client document · click to download</small></span></button>)}
          {selected.clarifications.flatMap((c) => c.documents || []).map((file) =>
          <button key={`clarification-${file.id}`} onClick={() => downloadWorkflowFile({ role: "agent", requestId: selected.id, kind: "clarification", fileId: file.id, name: file.name })} className="compact-document-card text-left"><span className="compact-document-icon"><FileText size={16}/></span><span className="min-w-0 flex-1"><strong>{file.name}</strong><small>Clarification document · click to download</small></span></button>)}</div></Card>
          <Card title="Agent output" icon={<Upload />}><div className="grid gap-2">{selected.workDocuments?.map((file) => <div key={file.id} className="compact-document-card"><span className="compact-document-icon"><FileText size={16}/></span><span className="min-w-0 flex-1"><strong>{file.name}</strong><small>Completed work document</small></span></div>)}</div>
